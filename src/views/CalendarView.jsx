    ...
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 28, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setFilterConc(v => !v)} style={{
          padding: '5px 14px', borderRadius: 2,
          background: filterConc ? T.alertBg : T.surface,
          color: filterConc ? T.alert : T.muted,
          border: `1px solid ${filterConc ? T.alert : T.line}`, cursor: 'pointer',
          fontFamily: fontTitle, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase',
        ...
        return (
          <div key={`${year}-${month}`} style={{ marginBottom: 56, position: 'relative' }}>
            ...
            <div style={{ border: `1px solid ${T.line}`, borderRadius: 4, overflow: 'hidden' }}>
              {/* Header row */}
              <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 1fr', background: T.ink }}>
                {['W', 'Commerciale', 'Brand'].map(h => (
                  <div key={h} style={{ padding: '8px 12px', fontFamily: fontTitle, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#fff', fontWeight: 700 }}>
                    {h}
                  </div>
                ))}
              </div>
              ...
              {filtered.map((w, idx) => {
                ...
                return (
                  <div key={`${w.year}-${w.week}`} style={{
                    ...
                    background: isCurrentWeek ? T.goldBg : (idx % 2 === 1 ? '#FAFAF8' : T.surface),
                    borderLeft: isCurrentWeek ? `3px solid ${T.gold}` : '3px solid transparent',
                    position: 'relative',
                  }}>
                    {/* Week number */}
                    <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 2, position: 'relative' }}>
                      <div style={{
                        position: 'absolute', right: 4, top: 6,
                        fontFamily: fontMono, fontSize: 32, fontWeight: 900,
                        color: T.ink, opacity: 0.04, lineHeight: 1, userSelect: 'none',
                      }}>{w.week}</div>
                      <span style={{ fontFamily: fontMono, fontSize: 13, color: T.ink, fontWeight: 600 }}>
                        W{w.week} {hasConc && <span title="Concomitanza">⚠️</span>}
                      </span>
                      <span style={{ fontFamily: fontMono, fontSize: 9, color: T.muted }}>{range}</span>
                    </div>

                    {/* Commercial */}
                    <div style={{ padding: '8px 10px', display: 'flex', flexWrap: 'wrap', alignContent: 'flex-start' }}>
